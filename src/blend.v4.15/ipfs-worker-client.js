...
function timeoutMsFromConfig(config = {}, key = 'timeoutMs', fallback = 30000) {
  const parsed = Number(config?.[key] ?? config?.timeoutMs);
  return Math.max(1000, Number.isFinite(parsed) && parsed > 0 ? parsed : fallback);
}
...
export class IpfsWorkerClient {
  ...
  async addFile(fileOrBlob, options = {}) {
    ...
    await this.init(config, options);
    return this.request(IPFS_WORKER_ACTION.ADD_FILE, {
      ...
      ...options,
      timeoutMs: options.timeoutMs || timeoutMsFromConfig(config, 'timeoutMs', 60000)
    });
  }
  ...
  async fetchCid(cid, options = {}) {
    ...
    const result = await this.request(IPFS_WORKER_ACTION.FETCH_CID, {
      ...
      ...options,
      timeoutMs: options.timeoutMs || timeoutMsFromConfig(config, 'fetchTimeoutMs', 90000)
    });
