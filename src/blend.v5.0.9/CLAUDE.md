...
The repo ships one self-contained copy of the app per release under `src/`, e.g. `src/blend.v5.0.4/`, `src/blend.v5.0.8/`, plus older `src/slideshow-playlist-player.v4.x/`. Each folder is a complete standalone app with its own `package.json`, tests, and assets — they are **siblings, not a shared codebase**. Work happens inside the latest folder; the current working directory (`src/blend.v5.0.8`) is the active one.

Both the root `README.md` (source of truth) and `.github/workflows/sanity-tests.yml` (CI) target `src/blend.v5.0.8`. Beware of other stale cross-references that trail the latest folder: root `VERSION` and `scripts/Start-PlayerDev.ps1` reference legacy paths.
...
All commands run from **inside a version folder** (e.g. `cd src/blend.v5.0.8`), which holds its own `package.json`.
