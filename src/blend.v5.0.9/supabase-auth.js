...
function describeAuthFailure(payload, status, friendlyCode) {
  const baseMessage = payload?.msg || payload?.error_description || payload?.error || `Supabase Auth request failed (${status})`;
  if (friendlyCode === 'auth_sign_in_failed') {
    const errorCode = String(payload?.error_code || '').toLowerCase();
    if (errorCode === 'invalid_credentials' || /invalid login credentials/i.test(baseMessage)) {
...
export function createSupabaseAuthClient({
  ...
  async function requestJson(url, init = {}, friendlyCode = 'auth_request_failed') {
    ...
    if (!response.ok) {
      const message = describeAuthFailure(payload, response.status, friendlyCode);
