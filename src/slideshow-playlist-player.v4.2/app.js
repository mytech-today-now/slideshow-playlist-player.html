import { attachGlobalErrorHandlers, createLogger } from './logger.js?v=20260611-mobilefix3';
import { createPointerReorderFallback } from './drag-sort.js?v=20260611-mobilefix3';
...
// =====================================================
// Blend • player.html v4.1.0  (Multi-Experience Workspaces, June 2026)
...
function syncExperienceControls() {
  ...
  $('#project-name').placeholder = 'Experience name';
...
function renderExperiencePicker() {
  ...
  const options = records.map(record => {
    ...
    const label = normalizeExperienceName(record.name || record.payload?.projectName || DEFAULT_EXPERIENCE_NAME);
    return `<option value="${escapeHtml(record.id)}"${isActive ? ' selected' : ''}>${escapeHtml(label)}</option>`;
  }).join('');
  select.innerHTML = options || `<option value="">No experiences</option>`;
...
async function hydrateState() {
  ...
  renderExperiencePicker();
  document.title = `${state.projectName} • Blend`;
  $('#experience-select').value = state.activeExperienceId || '';
}
...
async function loadPlaylistItem(itemRef, useB = false) {
  ...
  if (!item || item.stale || !item.handle) { advancePlaylist(); return; }
...
async function loadSlideshowItem(itemRef, withCrossfade = true) {
  const item = state.library.get(itemRef.id);
  if (!item || item.stale || !item.handle) { advanceSlideshow(); return; }
...
function wireConfig() {
  ...
  $('#project-name').oninput = (e) => {
    state.projectName = normalizeExperienceName(e.target.value);
    const current = currentExperienceRecord();
    if (current) updateExperienceCatalog(serializeActiveExperience(current));
    syncExperienceControls();
    renderExperiencePicker();
    saveStateDebounced();
  };
  $('#project-name').onblur = () => {
    state.projectName = ensureUniqueExperienceName(state.projectName, state.activeExperienceId);
    const current = currentExperienceRecord();
    if (current) updateExperienceCatalog(serializeActiveExperience(current));
    syncExperienceControls();
    renderExperiencePicker();
  ...
  $('#experience-select').onchange = async (e) => {
    const selectedId = e.target.value;
    const selectedLabel = e.target.selectedOptions?.[0]?.textContent || '';
    log.info('[experience] dropdown change', experienceDebugContext({
      selectedId,
      selectedLabel
    }));
    if (!selectedId) return;
    try {
      await switchExperienceById(selectedId);
    } catch (error) {
      log.error('[experience] dropdown switch failed', experienceDebugContext({
        selectedId,
        selectedLabel,
        error
      }));
...
async function materializeImportedEntries(entries, opts = {}) {
  ...
  const allowed = which === 'playlist' ? ['video', 'audio'] : ['video', 'image'];
  const seen = new Set((opts.existingIds || []).map(id => String(id)));
  ...
  for (const entry of entries || []) {
    ...
    if (!item && importedRecord) {
      item = ensureImportedLibraryRecord(importedRecord);
      id = item?.id || directId;
      if (!item && importedRecord.sourceUrl) {
        try {
          ...
          log.warn('Imported remote library record could not be fetched', error);
        }
      }
    }
    if (!id || !item) {
    ...
    if (seen.has(String(id))) { duplicate++; continue; }
    seen.add(String(id));
  ...
  return { items, missing, imported, incompatible, duplicate };
}
...
async function importExperienceFromFile(file) {
  try {
    ...
    const missing = playlist.missing.length + slideshow.missing.length;
    const detail = `${playlist.items.length} playlist item${playlist.items.length === 1 ? '' : 's'}, ${slideshow.items.length} slideshow item${slideshow.items.length === 1 ? '' : 's'}${missing ? `, ${missing} missing path${missing === 1 ? '' : 's'}` : ''}`;
