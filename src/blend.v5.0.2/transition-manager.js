...
const LIGHTWEIGHT_EFFECT_IDS = TRANSITION_EFFECTS
  .filter(effect => effect.id !== 'hard-cut' && !effect.heavy)
...
export function normalizeTransitionSettings(input = {}) {
  ...
  const enabledRaw = Array.isArray(source.enabledTransitionIds) ? source.enabledTransitionIds : DEFAULT_ENABLED_IDS;
  const enabled = enabledRaw
    .map(id => String(id || '').trim())
    .filter(id => EFFECT_BY_ID.has(id) && id !== 'hard-cut');
  const uniqueEnabled = Array.from(new Set(enabled));
  const enabledTransitionIds = uniqueEnabled.length ? uniqueEnabled : DEFAULT_ENABLED_IDS.slice();
...
export class TransitionManager {
  ...
  preferredEffectPool() {
    ...
    if (!ids.length) return [EFFECT_FALLBACK_ID];
    ...
    return lowTier.length ? lowTier : LIGHTWEIGHT_EFFECT_IDS;
  }
