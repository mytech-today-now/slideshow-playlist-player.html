...
test.describe('experience URL load progress overlay (v5.0.8)', () => {
  /**
  ...
  test('shows overlay, freezes transport, keeps config gear live, then cleans up on success', async ({ page, context }, testInfo) => {
    ...
    const shareUrl = await blendPage.buildShareLinkForExperience(experienceName);
    expect(shareUrl, 'share URL must contain the ?exp= param').toMatch(/[?&]exp=/);
  ...
  test('shows error state and does not auto-dismiss when payload is invalid', async ({ page, context }, testInfo) => {
    ...
    // Navigate with a deliberately broken ?exp= payload
    await receiverTab.goto('/index.html?exp=INVALID_GARBAGE_PAYLOAD', { waitUntil: 'commit' });
