...
async function resolvePlayableUrlForItem(item = null, ref = null, opts = {}) {
  ...
  try {
    ...
    if (shouldPromptForAuth(error) && (Date.now() - lastAuthPromptAt) > 4000) {
      ...
      showToast('Sign in to access private media in Supabase Storage.', {
        timeout: 4200,
        action: {
          label: 'Sign in',
...
function currentAuthReadiness() {
  ...
  if (!config.privateMediaRequiresAuth) {
    return {
      ...
      message: 'Private media auth is optional in this environment.'
    };
  }
  if (hasSession) {
    return {
      state: 'ready',
      canAccessPrivateMedia: true,
      message: 'Authenticated for private Supabase media.'
    };
  }
  return {
    state: 'unauthorized',
    canAccessPrivateMedia: false,
    message: 'Sign in to play private Supabase media.'
  };
}
...
function updateIpfsShareButtonState(readiness = currentAuthReadiness()) {
  ...
  shareBtn.title = connected ? 'Supabase session connected' : (readiness?.message || 'Sign in to Supabase');
  shareBtn.setAttribute('aria-label', connected ? 'Supabase session connected' : 'Sign in to Supabase');
}
...
function buildAuthDialog() {
  ...
  if (!form || !email || !password || !submit || !cancel) return null;
  return { dialog, form, email, password, error, submit, cancel };
}
...
async function promptSupabaseSignIn() {
  ...
  const { dialog, form, email, password, error, submit, cancel } = parts;
  ...
  return new Promise(resolve => {
    ...
    const onSubmit = async event => {
      ...
      error.textContent = '';
      try {
        const session = await supabaseAuthClient.signInWithPassword({
          email: email.value,
          password: password.value
        });
        syncIpfsControls();
        showToast('Signed in to Supabase.', { timeout: 2800 });
        finish(session);
      } catch (authError) {
        error.textContent = authError?.message || 'Sign-in failed. Check credentials and try again.';
        submit.disabled = false;
        cancel.disabled = false;
      }
    };
...
async function shareCurrentExperienceThroughIpfs() {
  ...
  if (!session) {
    showToast('Sign in is required to access private Supabase media.', { timeout: 3200 });
    return null;
  }
  syncIpfsControls();
  return session;
