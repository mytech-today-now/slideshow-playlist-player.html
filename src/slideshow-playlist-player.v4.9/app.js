import { attachGlobalErrorHandlers, createLogger } from './logger.js?v=20260619-v4.9-ipfs2';
import { createPointerReorderFallback } from './drag-sort.js?v=20260619-v4.9-ipfs2';
import {
  DEFAULT_IPFS_CONFIG,
  IPFS_GATEWAY_PARAM,
  IPFS_SHARE_PARAM,
...
const VERSION = '4.9';
const CACHE_VERSION = 'blend-player-v4.9-20260619-ipfs2';
...
const DEFAULT_SOCIAL_IMAGE = './assets/icon.svg';
const DEFAULT_SOCIAL_TYPE = 'website';
const DEFAULT_IPFS_WORKER_URL = './ipfs-worker.js?v=20260619-v4.9-ipfs2';
const DEFAULT_IPFS_HELIA_MODULE_URL = './ipfs-helia-provider.js?v=20260619-v4.9-ipfs2';
...
function ipfsConfigFromState(overrides = {}) {
  ...
  return normalizeIpfsConfig({
    ...
    heliaModuleUrl: state.settings.ipfsHeliaModuleUrl || '',
...
function syncIpfsControls() {
  ...
  if (helia) helia.value = state.settings.ipfsHeliaModuleUrl || '';
...
function buildIpfsExperienceShareUrl(manifestCid, config = ipfsConfigFromState()) {
  ...
  const firstGateway = parseGatewayList(config.gateways, []).at(0);
  if (firstGateway) url.searchParams.set(IPFS_GATEWAY_PARAM, firstGateway);
  return url.toString();
}
...
async function shareCurrentExperienceThroughIpfs(opts = {}) {
  ...
  for (const check of prepared.checks || []) {
    if (check?.ok === false) log.warn('IPFS provider preflight failed', check);
  }
  updateIpfsShareButtonState(readiness);
  ...
  try {
    ...
    for (const asset of collected.assets) {
      ...
      operation.update({
        ...
        detail: `${descriptor.cid} - ${formatBytes(descriptor.byteSize || asset.size)}`,
        index: step,
        total: totalSteps
      });
      await saveStateNow();
...
async function loadSharedIpfsExperience(request) {
  ...
  const config = ipfsConfigFromState(request.gateway ? { gateways: [request.gateway] } : {});
  const service = createIpfsService(config);
  const controller = new AbortController();
  ...
  try {
    ...
    for (const itemRecord of payload.libraryItems) {
      ...
      cidToId.set(cid, result.id);
      index++;
    }

    function refsFor(which, entries) {
      return entries.map(entry => {
        const id = cidToId.get(entry.cid);
        const item = id ? state.library.get(id) : null;
        if (!item) return null;
        const ref = createListRef(which, item, {
          path: `${ipfsSourceUrl(entry.cid)}/${item.name}`,
          sourceUrl: ipfsSourceUrl(entry.cid),
          name: entry.name,
          type: item.type,
...
function wireTransport() {
  ...
  if (ipfsHeliaInput) {
    ipfsHeliaInput.onchange = event => {
      state.settings.ipfsHeliaModuleUrl = String(event.target.value || '').trim();
      syncIpfsControls();
      saveStateDebounced();
    };
  }
