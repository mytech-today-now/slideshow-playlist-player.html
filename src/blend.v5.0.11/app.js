...
const CACHE_VERSION = PWA_CONFIG.CACHE_VERSION || '20260710-option4-v5.0.11';
 ...
 * Opens #url-share-modal with the URL, length check, and size diagnostics.
 */
async function shareExperienceViaUrl() {
  ...
  try {
    ...
    showUrlShareModal(shareUrl, payloadBytes, oversized, copied, payload, urlCheck, sizeReport);
  } finally {
...
function buildExperienceExportPayload(options = {}) {
  ...
  const referencedOnly = !!options.referencedOnly;
  const referencedIds = referencedOnly
    ? new Set([...playlistRefs, ...slideshowRefs].map(ref => ref?.id).filter(Boolean))
    : null;
  const libraryEntries = Array.from(state.library.entries())
    .filter(([id]) => !referencedIds || referencedIds.has(id));
  const libraryItems = sortLibraryEntries(libraryEntries)
 ...
 * @param {boolean} alreadyCopied   Whether clipboard write succeeded before opening
 ...
 */
function showUrlShareModal(shareUrl, payloadBytes, oversized, alreadyCopied, payload, urlCheck, sizeReport) {
  ...
  if (alreadyCopied && urlCheck.ok) {
    showToast('Share URL copied to clipboard', { timeout: 2600 });
  } else if (alreadyCopied && !urlCheck.ok) {
    showToast('URL copied — note it exceeds the 2 048-character browser limit', { timeout: 4000 });
  }
}
...
async function ensureRemoteUrlInLibrary(url, meta = {}) {
  ...
  if (existing) {
    ...
    scheduleMediaDurationDiscovery([existing]);
  ...
  scheduleMediaDurationDiscovery([item]);
...
function describeMediaLoadFailure(media, error, item, ref) {
  ...
  return { status, code, remote, transient, message };
}
...
function markRemoteMediaFailure(which, item, ref, failure) {
  ...
  showToast(`${itemName} is temporarily unavailable and was skipped.`, {
    timeout: 6200,
    action: {
      label: 'Retry now',
  ...
  if (announce) {
    ...
    showToast(`Removed ${count} item${count === 1 ? '' : 's'} from ${which}`, {
      undo: () => {
        ...
        state.ui.listSelection = new Set(removedIds);
        state.ui.listSelectionAnchorId = removed[0]?.id || null;
        renderListEditor();
        saveStateDebounced();
      }
    });
...
function renderListRow(idx, row) {
  ...
  if (row.dataset.signature !== signature) {
    ...
    if (which === 'slideshow') {
      ...
      if (entryType === 'image') {
        extra = `<input type="number" step="0.5" min="0" value="${dur}" title="Display seconds" aria-label="Display seconds">s`;
...
function wireListEditorEvents() {
  ...
  container.addEventListener('click', event => {
    ...
    if (event.target.closest('.del')) {
      event.stopPropagation();
      const selected = selectedListIndices(which);
      deleteSelectedListItems(which, selected.includes(idx) ? null : idx);
      return;
    }
...
async function loadPlaylistItem(itemRef, useB = false, opts = {}) {
  ...
  try {
    ...
    } else if (failure.transient && (Number(opts.retryAttempt) || 0) < REMOTE_MEDIA_MAX_RETRIES) {
...
async function loadSlideshowItem(itemRef, withCrossfade = true, opts = {}) {
  ...
  let preloaded;
  try {
    preloaded = await getPreloadedSlideshowElement(itemRef);
  } catch (error) {
  ...
  if (!preloaded?.el) {
    ...
    if (failure.transient && (Number(opts.retryAttempt) || 0) < REMOTE_MEDIA_MAX_RETRIES) {
      await wait(REMOTE_MEDIA_RETRY_DELAY_MS);
  ...
  if (item.type === 'image') {
    startKenBurns(el, slideshowDisplayDuration(itemRef));
  } else {
  ...
  if (item.type === 'image') {
    if (state.runtime.isPlaying) {
      const dur = slideshowDisplayDuration(itemRef) * 1000;
...
async function getPreloadedSlideshowElement(itemRef) {
  ...
  if (cached) {
    ...
    return cached.catch(error => ({ error, media: null }));
  }
