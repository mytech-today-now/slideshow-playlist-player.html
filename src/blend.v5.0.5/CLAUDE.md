...
The repo ships one self-contained copy of the app per release under `src/`, e.g. `src/blend.v5.0.4/`, `src/blend.v5.0.5/`, plus older `src/slideshow-playlist-player.v4.x/`. Each folder is a complete standalone app with its own `package.json`, tests, and assets — they are **siblings, not a shared codebase**. Work happens inside the latest folder; the current working directory (`src/blend.v5.0.5`) is the active one.

Both the root `README.md` (source of truth) and `.github/workflows/sanity-tests.yml` (CI) target `src/blend.v5.0.5`. Beware of other stale cross-references that trail the latest folder: root `VERSION` and `scripts/Start-PlayerDev.ps1` reference legacy paths.
...
All commands run from **inside a version folder** (e.g. `cd src/blend.v5.0.5`), which holds its own `package.json`.
...
- `drag-sort.js` — pointer-based reorder fallback for virtualized list rows.
...
### Persistence
- IndexedDB `player-blend-v1` (version 4): stores `library`, `playlist`, `slideshow`, `settings`, `experiences`, `thumbnails`, `dirHandles`. Bump `DB_VERSION` in `app.js` when changing the schema.
- LocalStorage: `blend-active-experience-id`, `blend-runtime-config-v1`, `blend-supabase-auth-session-v1`, consent/banner flags.
- Version constants live at the top of `app.js`: `VERSION`, `CACHE_VERSION` (service-worker shell cache key), `DB_VERSION`, `EXPERIENCE_EXPORT_SCHEMA`. The cache key string also appears in `index.html` asset query params and `service-worker.js` — keep them in sync on a release.

### Service worker
`service-worker.js` caches the app shell only. Media and HTTP `Range` requests intentionally bypass the cache and go to network/local handles. `sw.js` is a thin redirect to it.
