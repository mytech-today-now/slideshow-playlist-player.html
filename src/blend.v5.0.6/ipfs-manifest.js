...
function itemLocator(item = {}) {
  return sanitizeCid(item.cid) || sanitizeSourceUrl(item.sourceUrl);
}

function manifestItemFromUpload(upload) {
  const cid = sanitizeCid(upload?.cid);
  const sourceUrl = sanitizeSourceUrl(upload?.sourceUrl);
  const type = sanitizeText(upload?.type, 24).toLowerCase();
  if ((!cid && !sourceUrl) || !ALLOWED_ITEM_TYPES.has(type)) return null;
  ...
  if (!item.itemId) item.itemId = sourceUrl;
  return item;
}
...
function listEntriesFor(which, refs = [], uploadsById = new Map()) {
  return (Array.isArray(refs) ? refs : []).map((ref, index) => {
    ...
    if (upload.cid) entry.cid = upload.cid;
    if (upload.sourceUrl) entry.sourceUrl = sanitizeSourceUrl(upload.sourceUrl);
    if (which === 'slideshow') {
...
export function createExperienceManifest(input = {}) {
  ...
  for (const raw of uploadedItems) {
    ...
    if (!item) continue;
    const key = itemKey(item.itemId, itemLocator(item));
...
export function validateExperienceManifest(manifest, options = {}) {
  ...
  for (const [index, item] of (Array.isArray(manifest.items) ? manifest.items : []).entries()) {
    ...
    if (!cid && !sourceUrl) errors.push(`Item ${index + 1} needs a valid CID or sourceUrl`);
    if (item.cid != null && !cid) errors.push(`Item ${index + 1} has an invalid CID`);
    if (item.sourceUrl != null && !sourceUrl) errors.push(`Item ${index + 1} has an invalid sourceUrl`);
    ...
    if (!normalized.itemId) normalized.itemId = sourceUrl;
    ...
    if (normalized.cid) itemsByCid.set(normalized.cid, normalized);
    if (normalized.sourceUrl) itemsBySourceUrl.set(normalized.sourceUrl, normalized);
  }
  ...
  function normalizeList(which) {
    ...
    for (const [index, entry] of entries.entries()) {
      ...
      const referencedBySourceUrl = sourceUrl ? itemsBySourceUrl.get(sourceUrl) : null;
      const referenced = referencedByCid || referencedBySourceUrl || referencedById;
      if (!cid && !sourceUrl) errors.push(`${which} entry ${index + 1} needs a valid CID or sourceUrl`);
      if (entry.cid != null && (!cid || !validateCid(cid))) errors.push(`${which} entry ${index + 1} has an invalid CID`);
      if (entry.sourceUrl != null && !sourceUrl) errors.push(`${which} entry ${index + 1} has an invalid sourceUrl`);
      if (cid && !referencedByCid) errors.push(`${which} entry ${index + 1} references an unknown item CID`);
      if (sourceUrl && !referencedBySourceUrl) errors.push(`${which} entry ${index + 1} references an unknown sourceUrl`);
      ...
      if (referencedById && referencedBySourceUrl && referencedById.sourceUrl !== referencedBySourceUrl.sourceUrl) {
        errors.push(`${which} entry ${index + 1} itemId does not match its sourceUrl`);
      }
      ...
      normalizedEntries.push({
        ...
        itemId: referenced?.itemId || entryItemId || cid || sourceUrl,
        cid: cid || undefined,
...
export function manifestToImportPayload(manifest) {
  ...
  const itemByCid = new Map(safe.items.filter(item => item.cid).map(item => [item.cid, item]));
  const itemBySourceUrl = new Map(safe.items.filter(item => item.sourceUrl).map(item => [item.sourceUrl, item]));
  const libraryItems = safe.items.map(item => ({
    id: item.cid ? `ipfs-${item.cid}` : item.itemId,
    cid: item.cid,
    sourceUrl: item.sourceUrl || (item.cid ? `ipfs://${item.cid}` : ''),
    name: item.name || item.title || (item.cid ? `ipfs-${item.cid}` : 'Shared media'),
    type: item.type,
    size: item.byteSize || 0,
    duration: item.duration,
    mimeType: item.mimeType,
    pathHint: item.sourceUrl || (item.cid ? `ipfs://${item.cid}` : ''),
    metadata: {
      ...(item.metadata || {}),
      ...(item.cid ? { ipfs: {
        cid: item.cid,
  ...
  function entries(which) {
    return safe.lists[which].items.map(entry => {
      const item = entry.cid ? itemByCid.get(entry.cid) : itemBySourceUrl.get(entry.sourceUrl);
      return {
        id: item?.cid ? `ipfs-${item.cid}` : (item?.itemId || entry.itemId || entry.sourceUrl),
        cid: entry.cid,
        path: entry.sourceUrl || (entry.cid ? `ipfs://${entry.cid}` : ''),
        sourceUrl: entry.sourceUrl || (entry.cid ? `ipfs://${entry.cid}` : ''),
        name: entry.title || item?.title || item?.name || (entry.cid ? `ipfs-${entry.cid}` : 'Shared media'),
