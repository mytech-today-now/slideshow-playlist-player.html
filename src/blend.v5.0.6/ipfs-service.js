...
export const DEFAULT_IPFS_CONFIG = Object.freeze({
  ...
  workerUrl: './ipfs-worker.js?v=20260622-v4.15-ipfs-worker-timeout-fallback',
  useWorker: true,
...
export function normalizeIpfsConfig(config = {}) {
  ...
  const merged = { ...DEFAULT_IPFS_CONFIG, ...source };
  const hasExplicitHeliaConfigured = Object.prototype.hasOwnProperty.call(source, 'heliaConfigured');
  const mode = ['auto', 'kubo', 'helia', 'gateway'].includes(merged.mode) ? merged.mode : DEFAULT_IPFS_CONFIG.mode;
  ...
  const heliaModuleUrl = String(merged.heliaModuleUrl || '').trim();
  ...
  return {
    ...
    heliaModuleUrl,
    heliaConfigured: hasExplicitHeliaConfigured ? merged.heliaConfigured === true : !!heliaModuleUrl,
...
function isWorkerTimeoutError(error) {
  const code = String(error?.code || '').toLowerCase();
  const message = String(error?.message || '');
  if (code === 'ipfs_worker_timeout') return true;
  return /ipfs worker request timed out/i.test(message);
}

function shouldRetryRetrievalWithoutWorker(error, config = {}) {
  if (config?._workerDisabled || config?.useWorker === false) return false;
  const code = String(error?.code || '').toLowerCase();
  if (code === 'ipfs_worker_unavailable') return true;
  return isWorkerTimeoutError(error);
}
...
export function hasHeliaProviderCandidate(config = {}, scope = globalThis, options = {}) {
  const normalized = normalizeIpfsConfig(config);
  ...
  const canUseModule = !!normalized.heliaModuleUrl && (options.requireConfigured !== true || normalized.heliaConfigured === true);
  return !!(canUseInjected || canUseModule);
}
...
export function getIpfsPublishProviders(config = {}) {
  ...
  return uniqueProviders([
    ...
    hasHeliaProviderCandidate(normalized, globalThis, { requireConfigured: true }) ? 'helia' : ''
  ]);
}
...
async function runWithTimeout(promise, timeoutMs, message = 'IPFS request timed out') {
  const ms = Math.max(1000, Number(timeoutMs) || DEFAULT_IPFS_CONFIG.fetchTimeoutMs);
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      const error = new Error(message);
      error.name = 'TimeoutError';
      error.code = 'ipfs_timeout';
...
export function formatIpfsErrorForUser(error, config = {}) {
  ...
  if (error?.userMessage || error?.userDetail || error?.userAction) {
    return {
      title: error?.provider === 'helia' ? 'IPFS provider unavailable' : 'IPFS error',
      message: error.userMessage || 'IPFS sharing failed.',
      detail: error.userDetail || message || 'Check provider settings and retry.',
      action: error.userAction || '',
      code: error.code || 'ipfs_error',
      provider: error.provider || '',
      retryable: error.retryable !== false,
  ...
  if (isWorkerTimeoutError(error)) {
    return {
      title: 'IPFS request timed out',
      message: 'IPFS request timed out.',
      detail: 'The browser IPFS worker did not respond in time. Blend can retry with configured gateways or providers.',
      action: 'Retry the load, or increase IPFS timeout in Settings.',
      code: error?.code || 'ipfs_worker_timeout',
      provider: '',
      retryable: true,
      diagnostic: message
    };
...
export class IpfsService {
  ...
  async fetchCid(cid, options = {}) {
    if (shouldUseIpfsWorker(this.config, options)) {
      try {
        return await getIpfsWorkerClient({ workerUrl: this.config.workerUrl }).fetchCid(cid, {
          ...options,
          config: this.config
        });
      } catch (error) {
        if (!shouldRetryRetrievalWithoutWorker(error, this.config)) throw error;
        const fallbackService = new IpfsService({ ...this.config, useWorker: false, _workerDisabled: true });
        return fallbackService.fetchCid(cid, { ...options, _workerDisabled: true });
      }
    }
  ...
  async fetchJson(cid, options = {}) {
    if (shouldUseIpfsWorker(this.config, options)) {
      try {
        return await getIpfsWorkerClient({ workerUrl: this.config.workerUrl }).fetchJson(cid, {
          ...options,
          config: this.config
        });
      } catch (error) {
        if (!shouldRetryRetrievalWithoutWorker(error, this.config)) throw error;
        const fallbackService = new IpfsService({ ...this.config, useWorker: false, _workerDisabled: true });
        return fallbackService.fetchJson(cid, { ...options, _workerDisabled: true });
      }
    }
  ...
  async fetchCidWithHelia(cid, options = {}) {
    ...
    if (options.signal?.aborted) throw options.signal.reason || new DOMException('IPFS operation cancelled', 'AbortError');
    const providerFetchPromise = typeof provider.fetchCid === 'function'
      ? provider.fetchCid(cid, options)
      : typeof provider.cat === 'function'
        ? provider.cat(cid, options)
        : null;
    if (!providerFetchPromise) throw new Error('Configured Helia provider must expose fetchCid() or cat()');
    const value = await runWithTimeout(
      providerFetchPromise,
      options.timeoutMs || this.config.fetchTimeoutMs,
      `Helia fetch timed out after ${options.timeoutMs || this.config.fetchTimeoutMs}ms`
    );
    if (options.signal?.aborted) throw options.signal.reason || new DOMException('IPFS operation cancelled', 'AbortError');
  ...
  async fetchCidWithGateways(cid, options = {}) {
    const gateways = Array.isArray(this.config.gateways)
      ? this.config.gateways.filter(Boolean)
      : [];
    if (!gateways.length) throw new Error('No IPFS gateway is configured');

    const failures = new Array(gateways.length).fill('');
    const controllers = gateways.map(() => new AbortController());
    const cleanupAbortLink = [];

    if (options.signal) {
      const abortAll = () => {
        for (const controller of controllers) {
          try {
            controller.abort(options.signal.reason || new DOMException('IPFS operation cancelled', 'AbortError'));
          } catch (_) {
            controller.abort();
          }
        }
      };
      if (options.signal.aborted) abortAll();
      else {
        options.signal.addEventListener('abort', abortAll, { once: true });
        cleanupAbortLink.push(() => options.signal.removeEventListener('abort', abortAll));
      }
    }

    try {
      const attempts = gateways.map((gateway, index) => (async () => {
        const url = gatewayUrlForCid(gateway, cid);
        if (!url) throw new Error('Gateway URL is invalid for this CID');
        ...
        failures[index] = `${gateways[index]}: ${error?.message || error}`;
        throw error;
      }));

      const blob = await Promise.any(attempts);
      for (const controller of controllers) {
        if (controller.signal.aborted) continue;
        try {
          controller.abort(new DOMException('Gateway request superseded', 'AbortError'));
        } catch (_) {
          controller.abort();
        }
      }
      ...
      for (const cleanup of cleanupAbortLink) cleanup();
    }
  }
